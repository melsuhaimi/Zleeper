        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val classify = if (SleepClassifyEvent.hasEvents(intent)) SleepClassifyEvent.extractEvents(intent).orEmpty().map {
                    SleepSignalEntity(UUID.randomUUID().toString(), session.id, SleepSignalType.CLASSIFY.name, it.timestampMillis, null, it.confidence, receivedAt)
                } else emptyList()
                val segments = if (SleepSegmentEvent.hasEvents(intent)) SleepSegmentEvent.extractEvents(intent).orEmpty().map {
                    SleepSignalEntity(UUID.randomUUID().toString(), session.id, SleepSignalType.SEGMENT.name, it.endTimeMillis, it.segmentDurationMillis, null, receivedAt)
                } else emptyList()
