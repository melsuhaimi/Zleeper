...
import com.zleeper.sleepapp.game.simulation.CollisionPlatform
...
fun PlatformScene(
    ...
    val parallaxLayers = listOf("mid", "near").map { layer ->
        rememberAssetImage("game/atlas/region/region_${regionKey}_bg_${layer}.webp")
    }
    ...
    if (background == null || petAtlas == null || parallaxLayers.any { it == null }) {
        Box(modifier.fillMaxSize().background(ZleeperArtColors.DeepNight), contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Text("Opening ${scene.name}", style = MaterialTheme.typography.titleLarge)
                Text("Gathering the trail…", color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
        }
        return
    }

    val collisionPlatforms = remember(scene.id, scene.platforms) {
        scene.platforms.map { CollisionPlatform(it.x, it.y, it.width, it.height) }
    }
    val coyoteWindow = (GameSimulation.BASE_COYOTE_WINDOW + coyoteBonusSeconds.coerceAtLeast(0f))
        .coerceAtMost(GameSimulation.MAX_COYOTE_WINDOW)
    val simulation = remember(scene.id, coyoteWindow) {
        GameSimulation(collisionPlatforms, scene.worldWidth, scene.worldHeight, coyoteWindow)
    }
    ...
    var landingShake by remember(scene.id) { mutableFloatStateOf(0f) }
    var input by remember { mutableStateOf(GameInput()) }
    var completionSent by remember(scene.id) { mutableStateOf(false) }
    ...
    LaunchedEffect(scene.id, simulation, reducedMotion, screenShakeEnabled) {
        var previous = withFrameNanos { it }
        while (true) {
            val now = withFrameNanos { it }
            val delta = (now - previous) / 1_000_000_000f
            val next = simulation.step(player, input, delta)
            val landed = !player.grounded && next.grounded
            player = next
            if (!reducedMotion) sceneTime += delta.coerceIn(0f, 1f / 20f)
            landingShake = when {
                reducedMotion || !screenShakeEnabled -> 0f
                landed -> 1f
                else -> (landingShake - delta * 8f).coerceAtLeast(0f)
            }
            previous = now
            if (input.jump || input.interact) input = input.copy(jump = false, interact = false)
            if (!completionSent && player.x >= scene.worldWidth - GameSimulation.WIDTH - 80f) {
                completionSent = true
                onSceneCompleted()
            }
        }
    ...
    Box(modifier.fillMaxSize().background(ZleeperArtColors.DeepNight).testTag("platform-scene")) {
        Canvas(Modifier.fillMaxSize().semantics {
            contentDescription = "${scene.name} playable trail${trackedQuestText?.let { ", tracked quest $it" } ?: ""}"
        }) {
            ...
            val shakeY = if (landingShake > 0f) sin(sceneTime * 48f) * 5f * landingShake else 0f
            val ambientDrift = if (reducedMotion) 0f else sin(sceneTime * .25f) * 6f

            translate(top = shakeY) {
                ...
                drawImage(
                    background,
                    dstOffset = IntOffset((backdropX + ambientDrift).toInt(), 0),
                ...
                readyParallax.forEachIndexed { index, layer ->
                    ...
                    drawImage(
                        layer,
                        dstOffset = IntOffset((backdropX - extraTravel + ambientDrift * (index + 1)).toInt(), 0),
                ...
                if (!reducedMotion) {
                    repeat(7) { index ->
                        val phase = sceneTime * (.45f + index * .03f) + index * 1.7f
                        val x = size.width * (.08f + index * .135f) + sin(phase) * 18f
                        val y = size.height * (.18f + (index % 3) * .12f) + sin(phase * .73f) * 12f
                        drawCircle(ZleeperArtColors.Lantern.copy(alpha = .32f), 3.5f, Offset(x, y))
                    }
                }

                translate(left = -cameraX) {
                    scene.platforms.forEachIndexed { platformIndex, platform ->
                        ...
                        drawRoundRect(
                            ZleeperArtColors.PlatformBody,
                            Offset(left, top),
                            Size(width, height),
                            cornerRadius = androidx.compose.ui.geometry.CornerRadius(12f, 12f),
                        )
                        drawRoundRect(
                            ZleeperArtColors.PlatformEdge,
                            Offset(left, top),
                            Size(width, minOf(10f, height)),
                            cornerRadius = androidx.compose.ui.geometry.CornerRadius(12f, 12f),
                        )
                        var tuftX = left + 14f
                        while (tuftX < left + width - 10f) {
                            val sway = if (reducedMotion) 0f else sin(sceneTime * 1.8f + tuftX * .01f + platformIndex) * 3f
                            val grass = Path().apply {
                                moveTo(tuftX - 6f, top + 2f)
                                lineTo(tuftX + sway, top - 8f)
                                lineTo(tuftX + 6f, top + 2f)
                                close()
                            }
                            drawPath(grass, ZleeperArtColors.Grass)
                            tuftX += 34f
                        }
                    }

                    visibleInteractables.forEachIndexed { index, item ->
                        ...
                        val idleBob = if (isNpc && !reducedMotion) sin(sceneTime * 1.4f + index) * 3f else 0f
                        val y = item.y * worldScale - side + idleBob
                        if (image != null) {
                            val npcFrame = if (isNpc && !reducedMotion) ((sceneTime * 2.5f + index).toInt() % 3) else 0
                            drawImage(
                                image,
                        ...
                        if (nearest?.id == item.id) {
                            drawCircle(
                                ZleeperArtColors.Lantern.copy(alpha = .33f),
                                side * .48f,
                                Offset(item.x * worldScale, y + side * .5f),
                            )
                        }
                    }

                    val row = player.motion.atlasRow()
                    val fps = if (reducedMotion && player.motion == MotionState.IDLE) 0 else player.motion.framesPerSecond()
                    val frame = if (fps == 0) 0 else ((player.motionTimeSeconds * fps).toInt()).coerceAtLeast(0) % 8
                    ...
                    if (player.facingRight) drawPet()
                    else scale(-1f, 1f, pivot = Offset(spriteLeft + spriteSide / 2f, spriteTop + spriteSide / 2f)) { drawPet() }
                }
            }
        }
        ...
        Surface(
            ...
            Row(
                Modifier.padding(start = 6.dp, end = 16.dp, top = 4.dp, bottom = 4.dp),
                ...
                Column {
                    ...
                    Text(
                        scene.regionId.removePrefix("region_").replace('_', ' ').replaceFirstChar { it.uppercase() },
        ...
        trackedQuestText?.let { quest ->
            Surface(
                Modifier.align(Alignment.TopEnd).padding(top = 86.dp, end = 14.dp).testTag("tracked-quest-hud"),
                color = MaterialTheme.colorScheme.surface.copy(alpha = .86f),
                shape = RoundedCornerShape(18.dp),
            ) {
                Text(quest, Modifier.padding(horizontal = 12.dp, vertical = 8.dp), style = MaterialTheme.typography.labelMedium)
            }
        }
        ...
        nearest?.takeIf { interaction == null }?.let { target ->
            Surface(
                ...
                Text(
                    if (target.type == "NPC") "Talk" else "Collect",
        ...
        interaction?.let { message ->
            Surface(
                ...
                modifier = Modifier.align(Alignment.BottomCenter)
                    .padding(horizontal = 20.dp, vertical = if (largeControls) 118.dp else 104.dp),
                ...
                Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                    Surface(shape = CircleShape, color = MaterialTheme.colorScheme.secondaryContainer) {
                        Icon(
                            if (message.kind == WorldInteractionKind.DIALOGUE) Icons.Rounded.Pets else Icons.Rounded.KeyboardArrowUp,
                            null,
                    ...
                    Column(Modifier.weight(1f)) {
                        Text(message.title, style = MaterialTheme.typography.titleLarge)
                        Text(message.body, color = MaterialTheme.colorScheme.onSurfaceVariant, style = MaterialTheme.typography.bodyMedium)
                    }
        ...
        val movement: @Composable () -> Unit = {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                HoldControl(Icons.AutoMirrored.Rounded.ArrowBack, "Move left", "move-left", controlSize, controlOpacity) { pressed ->
                    if (pressed && hapticsEnabled) haptics.performHapticFeedback(HapticFeedbackType.TextHandleMove)
                    input = input.copy(horizontal = if (pressed) -1f else if (input.horizontal < 0f) 0f else input.horizontal)
                }
                HoldControl(Icons.AutoMirrored.Rounded.ArrowForward, "Move right", "move-right", controlSize, controlOpacity) { pressed ->
                    if (pressed && hapticsEnabled) haptics.performHapticFeedback(HapticFeedbackType.TextHandleMove)
                    input = input.copy(horizontal = if (pressed) 1f else if (input.horizontal > 0f) 0f else input.horizontal)
                }
            }
        ...
        val actions: @Composable () -> Unit = {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.Bottom) {
                if (nearest != null) {
                    ActionControl(
                        Icons.Rounded.Pets,
                        if (nearest.type == "NPC") "Talk" else "Collect",
                        "interact",
                        controlSize - 8.dp,
                        controlOpacity,
                    ) {
                        ...
                        onInteract(nearest.contentId)
                    }
                }
                ActionControl(Icons.Rounded.KeyboardArrowUp, "Jump", "jump", controlSize, controlOpacity) {
...
private fun MotionState.atlasRow(): Int = when (this) {
    ...
    MotionState.WALK -> 2
    MotionState.RUN -> 3
    MotionState.JUMP_START -> 4
    MotionState.JUMP_LOOP -> 5
    MotionState.FALL -> 6
    MotionState.LAND -> 7
    MotionState.INTERACT -> 8
}
...
private fun HoldControl(
    ...
    Surface(
        ...
        shape = CircleShape,
        color = if (pressed) MaterialTheme.colorScheme.primary.copy(alpha = opacity)
        else MaterialTheme.colorScheme.surface.copy(alpha = opacity),
        shadowElevation = if (pressed) 1.dp else 8.dp,
    ) {
        Box(contentAlignment = Alignment.Center) {
            Icon(icon, description, tint = if (pressed) MaterialTheme.colorScheme.onPrimary else MaterialTheme.colorScheme.onSurface)
        }
    }
}
...
private fun ActionControl(
    ...
    Surface(
        ...
        Box(contentAlignment = Alignment.Center) {
            Icon(icon, description, tint = MaterialTheme.colorScheme.onPrimary)
        }
    }
}
