...
- Begin Sleep seals the deterministic night identity by persisting the session timestamp and secure expedition seed; the materialized expedition/path/reward ledger is inserted during atomic morning resolution after finalized reach/region inputs are known
...
- Sleep API callback ingestion is limited to signal-eligible `ARMED`/`TRACKING` sessions; late callbacks cannot attach to `WAKE_PENDING` or `REVIEW_PENDING`
...
- boot restoration treats both `ARMED` and `TRACKING` sessions as eligible for automatic signal re-registration; a successfully restored `ARMED` subscription is promoted to `TRACKING`, while manual fallback remains valid if automatic estimation is unavailable
...
- persistence instrumentation definitions covering `ARMED` persistence, signal-eligible state filtering, `WAKE_PENDING` retry/reuse, duplicate signal collapse, one expedition per sleep session, Room transaction rollback, duplicate morning-resolution reward idempotency, database reopen preservation of active-night seed/wake anchor, and V1-to-V2 migration/data preservation
...
There is no remaining unresolved product/source-design rule blocking candidate freeze. The Begin-Sleep expedition recovery interpretation is resolved in `UNRESOLVED_DECISIONS.md` from the specific architecture contract plus the last fully validated Android baseline.

This repository status must not claim release or CI readiness until the following verification gates are closed:

1. Run an actual Android compiler pass for the current refined source tree, including Compose compiler, Hilt/Dagger code generation, Room/KSP, and migration integration.
2. Generate the authoritative Room V2 `2.json` schema from the compiler; do not fabricate it, then execute the defined V1-to-V2 migration validation against that generated schema.
3. Execute the defined JVM, persistence, Compose, content/schema, lint/static, instrumentation/device, and debug APK validation gates against one frozen exact commit.
4. Verify permission grant/denial/retry, exact alarm, notification delivery, boot restoration/re-registration, actual process-kill/restart recovery, real WorkManager retry behavior, navigation, and game rendering on Android runtime.
5. Add only Android-runtime evidence exposed by those runs; do not replace platform behavior with superficial unit-test substitutes.
